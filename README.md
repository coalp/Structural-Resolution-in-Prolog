...
We use the structural resolution interpreter to generate sentences of the formal language a^n b^{2m} c^{2m} d^n. The file [a_nb_2mc_2md_n.pl](a_nb_2mc_2md_n.pl) is a working Prolog program that does this work. For example, simply double click to open [a_nb_2mc_2md_n.pl](a_nb_2mc_2md_n.pl) on your PC, the SWI-Prolog interpreter would start. Then you give the query `?- s(A).`
